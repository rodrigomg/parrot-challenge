...
final class PaginationUtil {
  ...
  static <T> void generatePaginationHttpHeaders(MutableHttpHeaders headers, UriBuilder uriBuilder, Page<T> page) {
  ...
  private static String prepareLink(UriBuilder uriBuilder, int pageNumber, int pageSize, String relType) {
    MessageFormat.format(HEADER_LINK_FORMAT, preparePageUri(uriBuilder, pageNumber, pageSize), relType)
  }

  private static String preparePageUri(UriBuilder uriBuilder, int pageNumber, int pageSize) {
    uriBuilder.queryParam("page", pageNumber).queryParam("size", pageSize).build().toString()
  }
}
